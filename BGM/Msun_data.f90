...
do i= 1,3
    if (i .eq. 1)  filename = './BGM/star5000.dat'
    if (i .eq. 2)  filename = './BGM/star5000.dat'
    if (i .eq. 3)  filename = './BGM/star5000.dat'
...
do i = 1,3 
    ...
    allocate( read_array2(10, nt))    

    open(unit=1,file=filename,status='old')
    read(1,*) head
    read(1,*) head
    do j =1,nline(i)
    	read(1,*) dumm,(read_array2(k,j),k=1,10),indice_BGM
    ...
    do j=1,nt
		...
		Mass_startmp(i,j)	= read_array2(10,j)
