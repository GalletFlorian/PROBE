...
class sourceFile(object):
  ...
  COMPILATOR = "gfortran"
  OPTIONS = "-fdefault-real-8 -O3 -march=native "
    ...
    if (extra_files != None):
      ...
      for (source_file, object_file) in zip(extra_files, object_files):
        ...
        if isCompilation:
          ...
          if (ext == '.f90'):
            commande = sourceFile.COMPILATOR+" "+sourceFile.OPTIONS+" -c -fdefault-real-8 "+source_file
          elif (ext == '.c'):
            commande = "gfortran -c "+source_file
          else:
...
isProblem = False
problem_message = """AIM: Compilation of condinit programs.
By default, all of them, but one can specify one specific code to compile
and avoid the others (condinit, element, close). The compilation of dependances is automatic.
 ...
 * condinit : To compile condinit only
...
if (iscondinit):
  compile_source(filename="condinit.f90")
