...
subroutine TIDESTEST(hamlet,horatio,t_init,ap0,ep0,obls0,oblp0,wconvi,wradi,rotp0 &
    ...
    ,toto,radius,Qs_inv,a_roche,babaorum,spin_sat,Kwind,cst_diss,sigmas_eq,Sun_dyn_freq, &
                    Lum,d2Ldt,Teff,d2Teffdt,r2conv,d2r2convdt,r2rad,d2r2raddt,Mrad,d2Mraddt,Rrad,d2Rraddt, &
                    M_star,d2Massstardt, sizev, &
                    sizeder,ori,timedec,tideop,indice_BGM)
...
integer :: indice0,indicend,babaorum,cst_diss,tideop,indice_BGM,flag_print


real(dp), dimension(sizeder) :: d2RSdt,d2QSdt,d2Ldt,d2Teffdt,d2r2convdt,d2r2raddt,d2Mraddt,d2Rraddt,d2Massstardt
real(dp), dimension(sizev) :: Lum,Teff, tstar,r2conv,r2rad,Mrad,Rrad,toto,radius,Qs_inv,M_star
...
real(dp) :: Ls0,Teffs0, r2convs0,r2rads0,Rrads0,Mrads0,M_star0
real(dp) :: Ls,Teffs, r2convs,r2rads,Rrads,Mrads,M_stars
...
character :: filename1*100,filename2*100,filename3*100,filename4*100,filename5*100
...
real(dp),dimension(6) :: Ls_tabl,Teffs_tabl,r2convs_tabl,r2rads_tabl,Mrads_tabl,Rrads_tabl,rotstmp_tabl, M_star_tabl
real(dp) :: bb(6,6)
real(dp), dimension(:), allocatable :: t,a,e,oblp,obls,rots,rotp,aequ,enerdotp,enerdots,sigmast,tenerdots

integer :: ndata_line

real(dp) :: ttmp,ttmp2,atmp,etmp,oblptmp,oblstmp,rotstmp,rotptmp,aequtmp,sigmastmp


real(dp) :: wradi,wradf,Wuprad,juprad,deltaJ,Iconvtmp
real(dp) :: DAMbrake, DAMtide,AMtottmp,AMtotinit,DAMtot,DAMwind
real(dp) :: Wtide,Wupconv,Wdownconv,Wjuprad,deltaWc, M_star_out
...
call splint(toto,M_star,d2Massstardt,indicend-(indice0)+1,t_init/yr, M_star0)
...
do while (ttmp .le. Time)
    ...
    do int = 1, 6 
        ...
        if (ori .eq. 0) then
            ...
            call splint(toto,M_star,d2Massstardt,indicend-(indice0)+1,(ttmp+aa(int)*dt)/yr,M_stars)
            M_star_tabl(int) = M_stars
        endif
    ...
    if (ori .eq. 0) then
        ...
        if (rotstmp .ne. rotstmp) then
        	...
        	write(6,*) Mstarin2,Lum2,Teff2,tstar2,Rstar2,r2conv2,r2rad2,Mrad2,Rrad2 &
            ,Wuprad,juprad,deltaJ,timedec
            
			!open(unit=23,file=filename4,form='formatted',status='old')
			!write(23,*) (ttmp-t(1))/yr/1.e6, wconvi/2.87e-6,wradi/2.87e-6,Mstarin2(1)
			!close(23)
    ...
    if (flagcrash .eq. 1  ) then
     ...
     !.or. (ttmp .gt. 1.e10*yr) 
        ...
        if ( etmp .gt. 1.0d-98) then 
            ...
            grav = log10(M_star_tabl(5)*Msun*1.e3*G*1.e3/((Rs_tabl(5)*100)**2.))
            
            Jrad = wradf * r2rads_tabl(5) *M_star_tabl(5)*Msun*1000*(Rs_tabl(5)*100.0)**2.0
    		Jconv = rotstmp * r2convs_tabl(5)*M_star_tabl(5)*Msun*1000*(Rs_tabl(5)*100.0)**2.0 
    		
    		 
			open(unit=23,file=filename3,access = 'append',status='old')
            write(23,*) (ttmp-t(1))/yr/1.e6,rotstmp/2.87e-6,wradf/2.87e-6,M_star_tabl(5),Teffs_tabl(5),grav,Mrads_tabl(5) &
            			,Rrads_tabl(5),Rs_tabl(5),r2convs_tabl(5),r2rads_tabl(5)  &
						, Jconv,Jrad, indice_BGM 
            close(23)
            
            Irad = r2rads_tabl(5) *M_star_tabl(5)*Msun*1000*(Rs_tabl(5)*100.0)**2.0
    		Iconv = r2convs_tabl(5)*M_star_tabl(5)*Msun*1000*(Rs_tabl(5)*100.0)**2.0
    		
            DeltaJ_t = (Iconv*Jrad-Irad*Jconv) / (Iconv+Irad)
            term1 = DeltaJ_t / (Iconv*taudecinit*yr)
            dt_t = (tstar2(2) - tstar2(1))*yr
            dMrad = Mrads_tabl(5) - Mrads0
            
            dIconv = Iconv - r2convs0*M_star0*Msun*1000*(Rs0*100.0)**2.0
            
            term2 = (2.0 * (Rs_tabl(5)*100.0)**2.0 * rotstmp * dMrad*Msun*1000)/(3.0*Irad*dt_t)
            ...
            write(23,*) (ttmp-t(1))/yr/1.e6,DeltaJ,taudecinit,dt_t,dMrad*Msun*1000/dt_t,dIconv/dt_t,DAMbrake,term1,term2,term3,term4
            close(23)
            ...
            grav = log10(M_star_tabl(5)*Msun*1.e3*G*1.e3/((Rs_tabl(5)*100)**2.))
            Jrad = wradf * r2rads_tabl(5) *M_star_tabl(5)*Msun*1000*(Rs_tabl(5)*100.0)**2
    		Jconv = rotstmp * r2convs_tabl(5)*M_star_tabl(5)*Msun*1000*(Rs_tabl(5)*100.0)**2 
    ...
    if ( ttmp .ge. hamlet(kkk)*yr .and. flag_print .eq. 0 ) then 
        ...
        if ( e(kkk+1) .gt. 1.0d-98) then 
        	...
        	if (flagcrash .eq. 0) then
              ...
              open(unit=23,file=filename3,access = 'append',status='old')
           	  write(23,*) (t(kkk+1)-t(1))/yr/1.e6,rotstmp/2.87e-6,wradf/2.87e-6,M_star_tabl(5),Teffs_tabl(5),grav,Mrads_tabl(5) &
           	  			,Rrads_tabl(5),Rs_tabl(5),r2convs_tabl(5),r2rads_tabl(5)  &
						, Jconv,Jrad, indice_BGM        	  
              close(23)
              ...
              open(unit=23,file=filename5,access = 'append',status='old')
              write(23,*) (t(kkk+1)-t(1))/yr/1.e6,DeltaJ,taudecinit,dt_t,dMrad*Msun*1000/dt_t,dIconv/dt_t,DAMbrake,term1,term2 &
              				,term3,term4
              close(23)
            ...
            open(unit=23,file=filename5,access = 'append',status='old')
            write(23,*) (t(kkk+1)-t(1))/yr/1.e6,DeltaJ,taudecinit,dt_t,dMrad*Msun*1000/dt_t,dIconv/dt_t &
            			,DAMbrake,term1,term2,term3,term4
            close(23)
        ...
        if (kkk .ge. size(hamlet)) then
        	...
        	!open(unit=23,file=filename4,form='formatted',status='old')
			!write(23,*) (ttmp-t(1))/yr/1.e6, rotstmp/2.87e-6,wradf/2.87e-6,M_star_tabl(5)
			!close(23)
        	stop
        endif
