...
!       real(dp) :: taudecinit2,  tdiskparam2
      ...
      real(dp), dimension(2) :: Lum,Teff,tstar,Rstar
      ...
      REAL*8 RINIT, mstar2 ,DELTAWR,DELTAWC, deltaJ
!      real*8 TAUDEC
...
! Add of the Reiners & Mohanty 2012 braking law
        ...
        C = 2.66d3
        if (w0.lt.Wcr) then
          Wdownconv = C*((w0/Wcr)**4*w0*(r**16/(mstar2*msol)**2)**(1/3))* & 
          ((t1-t2)/Ic)   
        else
          Wdownconv = C*(w0*(r**16/(mstar2*msol)**2)**(1/3))*((t1-t2)/Ic)    
        end if
!----------------------------------------------------------
...
!      common/freinage/G,mdotstar,flag2,Bstar
      ...
      REAL*8 pi,rstar,mstar2,msol,rsol,fstar,period,grav,lgrav,Teff,G,M,L
